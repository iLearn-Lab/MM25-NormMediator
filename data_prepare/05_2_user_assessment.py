...
def run(i):
    ...
    while True:
        try:
            ...
            sys_prompt, user_prompt = user_assessment(agent_description, results[i]['social_network'][-1], results[i]['dialogue'][-1])
            ...
            if match:
                ...
                try:
                    ...
                    for k in extracted_dict.keys():
                        score = extracted_dict[k]['satisfaction_level'] + extracted_dict[k]['emotional_pressure']
                    if 'user_assessment' not in results[str(i)].keys():
                        results[str(i)]['user_assessment'] = [extracted_dict]
                    else:
                        results[str(i)]['user_assessment'] = results[str(i)]['user_assessment'] if isinstance(results[str(i)]['user_assessment'], list) else [results[str(i)]['user_assessment']]
                        results[str(i)]['user_assessment'].append(extracted_dict)
                    break
...
if __name__ == "__main__":
    ...
    input_file = o_dir / 'interaction_evaluation.json'
    output_file = o_dir / 'user_assessment.json' 
