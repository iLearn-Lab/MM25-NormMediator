...
def run(i):
    while True:
        sys_prompt, user_prompt = initialize_social_norm(results[i]['situation_description'])
        ...
        if match:
            ...
            try:
                ...
                results[str(i)]['social_norms_content'] = [d['content'] for d in extracted_dict.values()]
                results[str(i)]['social_norms'] = extracted_dict
                break
...
if __name__ == "__main__":
    ...
    input_file = o_dir / 'situation_agent.json'
    output_file = o_dir / 'initialize_social_norm.json'
