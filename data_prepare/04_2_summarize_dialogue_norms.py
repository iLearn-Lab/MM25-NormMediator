...
def run(i):
    ...
    while True:
        try:
            ...
            sys_prompt, user_prompt = summarize_dialogue_norms(results[i]['situation_description'], agent_description, results[i]['dialogue'][-1])
            ...
            if match:
                ...
                try:
                    ...
                    if 'dialogue_norms' not in results[str(i)].keys():
                        results[str(i)]['dialogue_norms'] = [extracted_dict] 
                    else:
                        results[str(i)]['dialogue_norms'] = results[str(i)]['dialogue_norms'] if isinstance(results[str(i)]['dialogue_norms'], list) else [results[str(i)]['dialogue_norms']]
                        results[str(i)]['dialogue_norms'].append(extracted_dict)
                    break
...
if __name__ == "__main__":
    ...
    input_file = o_dir / 'summary_update_properties.json'
    output_file = o_dir / 'summarize_dialogue_norms.json' 
