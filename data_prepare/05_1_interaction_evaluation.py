...
def run(i):
    ...
    while True:
        try:
            sys_prompt, user_prompt = interaction_evaluation(results[i]['dialogue'][-1])
            ...
            if match:
                ...
                try:
                    ...
                    if 'conflict_level' in extracted_dict.keys() and 'emotional_intensity' in extracted_dict.keys():
                        if 'interaction_evaluation' not in results[str(i)].keys():
                            results[str(i)]['interaction_evaluation'] = [extracted_dict]
                        else:
                            results[str(i)]['interaction_evaluation'] = results[str(i)]['interaction_evaluation'] if isinstance(results[str(i)]['interaction_evaluation'], list) else [results[str(i)]['interaction_evaluation']]
                            results[str(i)]['interaction_evaluation'].append(extracted_dict)
                        break
...
if __name__ == "__main__":
    ...
    input_file = o_dir / 'summarize_dialogue_norms.json'
    output_file = o_dir / 'interaction_evaluation.json' 
